...
idxall = find(q);
idxvary = sub2ind(size(q), [1 4 13 6 2 5 5 6 6 10 10 11 11], [4 1 6 13 5 2 4 9 4 7 12 9 12]);
...
% Constrain the loop containing states 1, 4, 6, and 13
% src = sub2ind(size(q),[1 4 6 13],[4 6 13 1]); %Anti-clockwise
% tgt = sub2ind(size(q),[1 13 6 4],[13 6 4 1]); %Clockwise
% [tmpg,tmpxi] = constrainrate(q,idxall,'loop',src,tgt);
% gamma=[gamma;tmpg];
% xi=[xi;tmpxi];
...
tgt = sub2ind(size(q), [3 11 13 9], [5 8 1 6]);
[tmpg,tmpxi] = constrainrate (q, idxall, 'constrain', src, tgt, [1 1 1 2]);
gamma=[gamma;tmpg];
xi=[xi;tmpxi];
