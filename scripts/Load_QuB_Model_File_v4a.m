%% Get Qub Model Files (qmf)
[qmffile,qmfpath]=uigetfile({'*.qmf','QuB Model File'},'MultiSelect','on');
if ~iscellstr(qmffile)
    if ischar(qmffile)
        qmffile = cellstr(qmffile);
    else
        error('The Qub Model File name should be a cell string. Try entering it manually');
    end
end
...
write_qmatrix_results(filename,[path file{ii}],qnew{ii},A,F,idxall,idxvary,...
    td,tcrit(ii),nDwells(ii),runtime(ii),ll(ii),eq_Po{ii},opentaus{ii},...
